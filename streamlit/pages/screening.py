...
from Final_project.streamlit.theme import apply_theme # 앱 theme
...
with col_right:
    ...
    # =========================
    #  SHAP 핵심요인(피처) 한글 라벨 매핑
    # =========================
    SHAP_FEATURE_KR = {
        "is_n_offices_missing": "오피스 수 결측 여부",
        "is_city_missing": "도시 정보 결측 여부",
        "subject_group": "전공 그룹",
        "degree_level_filled": "최종 학위 수준(보정)",
        "is_inst_missing": "학교 정보 결측 여부",
        "n_offices": "오피스 수",
        "inst_tier": "학교 티어",
        "log1p_n_founding": "창업 경험 수(log1p)",
        "reinvest_rate_next": "후속 라운드 재투자율",
        "city_grouped": "도시 그룹",
        "is_n_founding_missing": "창업 경험 결측 여부",
        "log1p_time_to_first_funding_days": "첫 투자까지 기간(log1p)",
        "first_participants": "첫 라운드 투자자 수",
        "log1p_first_raised_amount": "첫 라운드 투자금(log1p)",
        "relationships": "관계 규모(네트워크)",
    }
    # with tab1:
    #     st.markdown(
    #         f"""
        ...
        if shap_local.empty:
            ...
            if shap_df.empty:
                ...
                if shap_long.empty:
                    ...
                    # shap_long["feature"] -> shap_long["feature_kr"]로 변환(없는 건 원문 유지)
                    shap_long["feature_kr"] = shap_long["feature"].map(SHAP_FEATURE_KR).fillna(shap_long["feature"])
